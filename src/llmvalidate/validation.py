...
def _confidence_column(field, columns):
    """The `Res: ... confidence` column used to bin `field`, or None.

    Prefer the field's own `Res: <field> confidence`. If absent and `<field>` is a coded
    facet (`<base>-value` / `<base>-code`), fall back to the parent `Res: <base> confidence`
    so both facets of a coded concept share the single confidence column emitted by
    flatten_structured_result. Non-facet fields are unaffected (they only use the direct
...
def get_metrics(res_df, fields):
    ...
    for field in fields:
        confidence_col = _confidence_column(field, clean_df.columns)
        if confidence_col:
    ...
    for field in fields:
        confidence_field_name = _confidence_column(field, clean_df.columns)
        has_confidence = confidence_field_name is not None
...
def bootstrap_CI(
    ...
    """
    Calculate bootstrap confidence intervals for validation metrics.
    
    This function performs bootstrap resampling to estimate confidence intervals
    for validation metrics across specified fields. For each bootstrap iteration,
    it samples with replacement from the input dataframe, calculates metrics,
    and then aggregates all bootstrap samples to compute confidence intervals.
    
    Parameters:
    -----------
    res_df : pd.DataFrame
        Results dataframe containing validation results with comparison columns
        (e.g., 'Cor: field', 'Inc: field', etc.) generated by validate.
        
    fields : list of str
        List of field names for which to calculate confidence intervals.
        These should correspond to columns in res_df.
        
    n_bootstrap : int, default=5000
        Number of bootstrap iterations to perform. Higher values provide
        more stable confidence intervals but take longer to compute.
        
    ci : float, default=0.95
        Confidence interval level (between 0 and 1). Default is 0.95 for
        95% confidence intervals.
        
    random_state : int or None, default=42
        Random seed for reproducible bootstrap sampling. Set to None for
        non-deterministic results.
        
    Returns:
    --------
    pd.DataFrame
        DataFrame with confidence intervals for each metric and field combination.
        Columns include:
        - 'field': Field name
        - '{metric}: mean': Bootstrap mean for each metric
    ...
    -------
    ValueError
        - If ci is not between 0 and 1
        - If res_df has fewer than 2 rows
        - If any specified fields contain missing labels (NaN values)
    """
    if not (0 < ci < 1):
    ...
    n = len(res_df)
    if n < 2:
        raise ValueError("Need at least 2 rows in res_df for bootstrap.")

    if not fields:
        fields = infer_fields(res_df)

    # Validate that there are no missing labels (NaN) in specified fields
    missing_fields = [field for field in fields if res_df[field].isna().any()]
    if missing_fields:
        raise ValueError(f"Missing labels (NaN) found in the following fields: {missing_fields}")
    ...
    # Collect all sample_metrics_df in a list
    all_metrics = []
    for b in range(n_bootstrap):
        idx = rng.integers(0, n, size=n)
        sample_df = res_df.iloc[idx]
        sample_metrics_df = get_metrics(sample_df, fields)
        all_metrics.append(sample_metrics_df)

    # Concatenate all sample_metrics_df vertically
    combined_df = pd.concat(all_metrics, ignore_index=True)

    # For each unique field, calculate CI for each other column (excluding 'field')
    result_rows = []
    metric = [col for col in combined_df.columns if col not in ('field')]

    # Preserve the order of fields as they appear in combined_df
    seen_fields = set()
    ordered_fields = []
    for field in combined_df['field']:
        if field not in seen_fields:
            ordered_fields.append(field)
            seen_fields.add(field)

    for field in ordered_fields:
        field_df = combined_df[combined_df['field'] == field]
        row = {'field': field}
        for col in metric:
            arr = field_df[col].dropna().values.astype(float)
            if arr.size == 0:
                continue

            if col == 'labeled cases':
                row[f'labeled cases'] = float(np.mean(arr)) #all cases are labeled for all fields
            else:
                row[f'{col}: mean'] = float(np.mean(arr))
                row[f'{col}: lower'] = float(np.percentile(arr, q_lo))
